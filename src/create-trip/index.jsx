...
function CreateTrip() {
  ...
  const OnGenerateTrip=() => {
    if(formData?.noOfDays>5&&!formData?.location||!formData?.budget||!formData?.travelers){
      toast("Please fill all the fields to continue")
      return;
    }
    console.log('FormData', formData)
  }
  ...
  return (
    <div className='sm:px-10 md:px-32 lg:px-56 xl:p-10 px-5 mt-10'>
      ...
      <p className='mt-3 text-gray-500 text-xl'>Just provide some basic information, and our trip planner will design your trip accordingly</p>
      ...
      <div>
        ...
        <div className='grid grid-cols-3 gap-5 mt-5'>
          {SelectBudgetOptions.map((item, index) => (
            <div key={index} onClick={()=>handleInputChange('budget', item.title)}  // Add onClick event to handle budget selection
      ...
      <div>
        ...
        <div className='grid grid-cols-3 gap-5 mt-5'>
          {SelectTravelesList.map((item, index) => (
            <div key={index} onClick={()=>handleInputChange('travelers', item.title)} // Add onClick event to handle travelers selection
             className={`p-4 border cursor-pointer rounded-lg hover:shadow-lg
